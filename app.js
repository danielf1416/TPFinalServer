...
app.get('/api/verlibros/:id', async (req, res) => {
    try {

        const respuesta = await qy('SELECT * FROM libro WHERE categoria_id = ?', [req.params.id]);
        if (respuesta.length == 0) {
            res.status(413).send({ "Error": "No hay Libros para esa Categoria" });
            // throw new Error('No hay Libros para esa Categoria');
        } else {
            // res.send(respuesta);
            res.send(respuesta);
        }
...
app.get('/api/verlibrosprestados/:id', async (req, res) => {
    try {

        const respuesta = await qy('SELECT * FROM libro WHERE persona_id = ?', [req.params.id]);
        if (respuesta.length == 0) {
            res.status(413).send({ "Error": "No hay Libros Prestados a esta persona" });
            // throw new Error('No hay Libros para esa Categoria');
        } else {
            // res.send(respuesta);
            res.send(respuesta);
...
app.post('/api/libro', async (req, res) => {
    try {
        ...
        //guardo nuevo libro
        consulta = 'INSERT INTO libro (nombre, descripcion, categoria_id) VALUES (?, ?, ?)';
        respuesta = await qy(consulta, [nombre, descripcion, req.body.categoria_id]);
...
app.get('/api/libroconpresona', async (req, res) => {
    try {
        const respuesta = await qy('SELECT libro.*, persona.alias, persona.nombre as nombre_persona, persona.apellido FROM libro left join persona on persona.id = libro.persona_id');
        console.log(respuesta);
        res.send(respuesta);
...
app.get('/api/libroconpersonaycategoria/:id', async (req, res) => {
    try {

        const respuesta = await qy('SELECT libro.id, libro.nombre, libro.descripcion, libro.categoria_id, libro.persona_id, categoria.nombre as nombre_categoria, persona.alias FROM libro left join categoria on categoria.id = libro.categoria_id left join persona on persona.id = libro.persona_id WHERE libro.id = ?', [req.params.id]);
        if (respuesta.length == 0) {
